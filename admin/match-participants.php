...
if(isset($_GET['matchId']))
{
  ...
  $txtCdate=date("Y-m-d H:m:s");
    ...
    if(isset($_POST['btnAddParti']))
    {
      ...
      $txtPbid = $_POST['txtPbid'];
      
      $selquery ="select max(slot) from participant_details where match_id=$matchId";
      $selresult = mysqli_query($conn,$selquery);

      $acountquery="select count(*) from participant_details where match_id=$matchId";
      ...
      {
        $amax = "select max(slot) from participant_details where match_id=$matchId";
      ...
      for($i=0;$i<count($txtPbid);$i++)
      {
          ...
          if($pubId != '')
          {
              ...
              {
                $chkBal ="select cur_balance, won_balance from user_details where id=$txtUid";
                $selresultCB = mysqli_query($conn,$chkBal);
                $selresCB = mysqli_fetch_array($selresultCB);

                if($selresCB['cur_balance'] >= $selres1M['entry_fee'])
                {
                  $selquery ="select * from participant_details where pubg_id='$pubId' and match_id='$matchId'";
                  $selresult = mysqli_query($conn,$selquery);
                  if($selres = mysqli_fetch_array($selresult))
                  {
                      echo "<script>alert(\"$pubId is already register\");</script>";
                      //header("Location:match-participants.php?matchId=".$matchId);
                  }
                  else
                  {
                    $insquery = "insert into participant_details (match_id, user_id, pubg_id, kills, win, position, prize, created, slot) values('$matchId','{$txtUid}','{$pubId}',0,0,0,0,'{$txtDate}',$slot)";
                    if(mysqli_query($conn,$insquery))
                    {
                      if($selresCB['won_balance']>=$selres1M['entry_fee'])
                      {
                        $upquery = "update user_details set cur_balance=cur_balance-".$selres1M['entry_fee'].", won_balance=won_balance-".$selres1M['entry_fee']." where id={$txtUid}";
                      }                      
                      else
                      {
                        $upquery = "update user_details set cur_balance=cur_balance-".$selres1M['entry_fee'].", won_balance=0 where id={$txtUid}";
                      }
                ...
                else
                {
                  echo '<script>
                          setTimeout(function() {
                              swal({
                                  title: "Oops..",
                                  text: "Insufficient Coin to join match..",
                                  type: "error"
                              }, function() {
                                  window.location = "match-participants.php?matchId='.$matchId.'";
                              });
                          }, 1000);
                      </script>';
                  break;
                        ...
                        <div class="row">
                            <div class="col-sm-10">
                                ...
                                <p class="text-muted font-13 m-b-30">
                                    Add or remove participant of match.
                                </p>
                            </div>
                        </div>
                          ...
                          <table id="datatable-responsive1" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                              ...
                              <tbody>
                                <?php while ($selresP = mysqli_fetch_array($selresultP)){ ?>
                                  <tr>
                                      <td>
                                        <a href="match-participants.php?matchId=<?php echo $matchId; ?>&partiid=<?php echo $selresP['id']; ?>" class="btn btn-danger btn-xs" data-target="#myModal2" data-toggle="tooltip" data-placement="right" title="" data-original-title="Remove Participant"> <i class="fa fa-long-arrow-left"></i> </a>
                                      </td>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <form role="form" action="match-participants.php?matchId=<?php echo $_GET['matchId'];?>" method="post" data-parsley-validate novalidate>
                    <input type="hidden" name="txtUid" id="txtUid">
                      <?php if ($selres1M['match_type']=='Solo') { ?>
                      <div class="form-group">
                          <label for="txtPbid">Enter PubG Id</label>
                          <input type="text" required class="form-control" id="txtPbid" name="txtPbid[]" value="">
                      </div>
                      <?php } else if ($selres1M['match_type']=='Duo') { ?>
                      <div class="form-group">
                          <label for="txtPbid">Enter PubG Id</label>
                          <input type="text" required class="form-control" id="txtPbid" name="txtPbid[]" value="">
                      </div>
                      <div class="form-group">
                          <label for="txtPbid">Enter PubG Id</label>
                          <input type="text" class="form-control" id="txtPbid" name="txtPbid[]" value="">
                      </div>
                      <?php } else if($selres1M['match_type']=='Squad') { ?>
                        <div class="form-group">
                            <label for="txtPbid">Enter PubG Id</label>
                            <input type="text" required class="form-control" id="txtPbid" name="txtPbid[]" value="">
                        </div>
